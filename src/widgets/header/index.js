...
const Header = (_a) => {
    var { variant = 'static', logo, background } = _a, props = __rest(_a, ["variant", "logo", "background"]);
    const loginData = localStorage.getItem('loginData');
    const userName = loginData ? JSON.parse(loginData).username : null;
    const rootClasses = ['header'];
    if (variant)
        rootClasses.push(`header-${variant}`);
    return (react_1.default.createElement("header", Object.assign({}, props, { className: rootClasses.join(' '), style: { background } }),
        react_1.default.createElement("img", { src: logo, className: "logo" }),
        react_1.default.createElement("div", { className: "right-block" },
